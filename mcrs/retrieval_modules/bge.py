...
class BGE_MODEL:
    ...
    def __init__(self,
        ...
        self.index_dir = os.path.join(self.cache_dir, "bge", self.corpus_name)
    ...
    def build_index(self) -> None:
        ...
        with torch.no_grad():
            for start in range(0, len(corpus_texts), self.batch_size):
                ...
                pooled = outputs.last_hidden_state[:, 0] # [CLS] token embedding as global representation
    ...
    def batch_text_to_item_retrieval(self, queries: List[str], topk: int) -> List[List[str]]:
        ...
        with torch.no_grad():
            ...
            batch = {k: v.to(self.device) for k, v in batch.items()}
            outputs = self.model(**batch)
            query_embs = outputs.last_hidden_state[:, 0]
